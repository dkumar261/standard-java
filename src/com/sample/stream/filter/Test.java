...
public class Test {
	...
	// Ex-1 Find product with category Book which is price more than 100 with double
	// filter
	private static List<Product> getProductWithBookAndPriceGreaterThen100WithFilter(List<Product> products) {
		return products.stream().filter(product -> product.getPrice() > 100)
				.filter(product -> product.getCategory().equals("Book")).collect(Collectors.toList());
	}
	...
	// Exercise 4 — Obtain a list of products ordered by customer of tier 2 between
	// 01-Feb-2021 and 01-Apr-2021
	private static List<Product> getProductWithCustomerTier2(List<Order> orders) {

		List<Product> productList = orders.stream().filter(order -> order.getCustomer().getTier() == 2)
...
//		}
	...
	private static List<Product> getProducts() {
		List<Product> products = Arrays.asList(new Product("Laptop", 10000, "HP"),
				...
				new Product("Ram", 1800, "Samsung"), new Product("Book", 800, "HTML"), new Product("Baby", 800, "Toy"),
				new Product("Toy", 8200, "Toy-Elephant"), new Product("Toy", 8100, "Toy-Tiger"),
				new Product("Toy", 8300, "Toy-Fish"), new Product("Mobile", 500, "OnePlus"));
	...
	private static List<Order> getOrders() {
		...
		List<Product> products = Arrays.asList(new Product("Laptop", 10000, "HP"),
				new Product("Book", 110, "JavaScript"), new Product("Book", 120, "Microservice"),
				new Product("Baby", 110, "Spring in Action"), new Product("Book", 80, "RDBMS"),
				new Product("Mouse", 180, "RDBMS"), new Product("keyboard", 80, "RDBMS"),
